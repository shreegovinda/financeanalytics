...
function getOtpAttemptKey(req, email, purpose) {
  return `${purpose}:${email.toLowerCase()}:${req.ip}`;
}

function checkOtpRateLimit(req, email, purpose) {
  const key = getOtpAttemptKey(req, email, purpose);
  const now = Date.now();
...
function clearOtpRateLimit(req, email, purpose) {
  otpAttempts.delete(getOtpAttemptKey(req, email, purpose));
}
...
router.post('/forgot-password/send-otp', async (req, res) => {
  ...
  try {
    ...
    await sendOTP(email, userResult.rows[0].name || 'User', 'password_reset');
...
router.post('/forgot-password/reset', async (req, res) => {
  ...
  try {
    if (!checkOtpRateLimit(req, email, 'password_reset')) {
      return res
        .status(429)
    ...
    clearOtpRateLimit(req, email, 'password_reset');
...
router.post('/send-otp', async (req, res) => {
  ...
  try {
    ...
    // Send OTP
    await sendOTP(email, name, 'login');
...
router.post('/verify-otp', async (req, res) => {
  ...
  try {
    // Verify OTP
    if (!checkOtpRateLimit(req, email, 'login')) {
      return res
        .status(429)
        .json({ error: 'Too many invalid OTP attempts. Please try again later.' });
    }
    ...
    clearOtpRateLimit(req, email, 'login');
  } catch (err) {
