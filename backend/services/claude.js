...
async function categorizeWithRetry(transactions, providerId, startIndex = 0, attempt = 0) {
...
Respond ONLY with valid JSON array, no other text.`;
    ...
    const results = categorizations
      .map((cat) => {
        const batchIndex = Number(cat.index) - 1;

        if (!Number.isInteger(batchIndex) || batchIndex < 0 || batchIndex >= transactions.length) {
          return null;
        }
        ...
        return {
          transactionIndex: startIndex + batchIndex,
    ...
    if (attempt < MAX_RETRIES - 1) {
      ...
      return categorizeWithRetry(transactions, provider, startIndex, attempt + 1);
    }
    throw err;
  }
}
