...
function CategoryCellRenderer({ data }: ICellRendererParams<TxnRow>) {
  if (!data) return null;
  if (!data.parentName) {
    return <span className="text-gray-400 italic text-xs">{data.aiSuggested || 'Uncategorized'}</span>;
  }
  return (
    <div className="flex items-center gap-1.5">
      <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: data.parentColor }} />
...
function SubcategoryCellRenderer({ data }: ICellRendererParams<TxnRow>) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      <span className="w-2.5 h-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: data.subColor }} />
...
type FilterMode = 'all' | 'this_month' | 'last_month' | 'this_quarter' | 'this_fy' | 'last_fy' | 'custom';
...
export default function TransactionsPage() {
  ...
  const resolveCats = useCallback((categoryId: string | undefined) => {
  ...
  const rowData = useMemo<TxnRow[]>(() => transactions.map((t) => {
  ...
  const categoryBreakdown = useMemo(() => {
    ...
    return [...map.entries()].map(([id, d]) => ({ id, ...d })).sort((a, b) => b.total - a.total).slice(0, 7);
  }, [transactions, categories]);

  const maxCatTotal = categoryBreakdown[0]?.total ?? 1;

  const periodLabel = useMemo(() => {
    if (filterMode === 'custom' && appliedStart && appliedEnd) return `${appliedStart} → ${appliedEnd}`;
  ...
  const saveCategory = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001';
      await apiPut(`${apiUrl}/api/transactions/${editingTxnId}`, { categoryId }, token ?? undefined);
      setTransactions((prev) => prev.map((t) => t.id === editingTxnId ? { ...t, category_id: categoryId } : t));
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-5">
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-700 text-sm px-4 py-3 rounded-lg flex items-center justify-between">
            <span>{error}</span>
            <button onClick={() => setError('')} className="text-red-400 hover:text-red-600 cursor-pointer ml-3">✕</button>
          </div>
        )}

        {/* ── Filter panel ── */}
        <div className="bg-white rounded-xl border border-gray-200 p-4 space-y-3">
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Filter Period</p>
          ...
          {filterMode === 'custom' && (
            <div className="flex flex-wrap gap-3 items-end pt-1">
              ...
              <div>
                ...
                <input type="date" max={today} min={customStart || undefined} value={customEnd} onChange={(e) => setCustomEnd(e.target.value)}
                  className="px-3 py-2 text-sm text-gray-900 bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none cursor-pointer" />
              </div>
              ...
              <button onClick={() => { setCustomStart(''); setCustomEnd(''); setAppliedStart(''); setAppliedEnd(''); }}
        ...
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {[1,2,3,4].map((i) => <div key={i} className="bg-white rounded-xl border border-gray-200 p-4 h-24 animate-pulse" />)}
          </div>
        ) : (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-xs text-gray-500 mb-1">Income</p>
              <p className="text-xl font-bold text-green-600">₹{stats.income.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              <p className="text-xs text-gray-400 mt-1">{periodLabel}</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-xs text-gray-500 mb-1">Expenses</p>
              <p className="text-xl font-bold text-red-600">₹{stats.expenses.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
              <p className="text-xs text-gray-400 mt-1">{periodLabel}</p>
            </div>
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              <p className="text-xs text-gray-500 mb-1">Net Savings</p>
              <p className={`text-xl font-bold ${stats.net >= 0 ? 'text-blue-600' : 'text-red-500'}`}>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="space-y-3">
              {categoryBreakdown.map((cat) => {
                ...
                const share = stats.expenses > 0 ? Math.round((cat.total / stats.expenses) * 100) : 0;
                return (
                  <div key={cat.id}>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-sm font-medium text-gray-800">₹{cat.total.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                      </div>
                    </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <span className="text-xs text-gray-400">{stats.count} records · click column header to sort · filter in search boxes below headers</span>
              <button
                onClick={() => gridApi?.exportDataAsCsv({ fileName: `transactions-${periodLabel}.csv` })}
      ...
      {editingTxnId && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50"
          onClick={(e) => { if (e.target === e.currentTarget) setEditingTxnId(null); }}>
          <div className="bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm mx-4 border border-gray-100" role="dialog" aria-modal="true">
            <div className="flex items-center justify-between mb-5">
              <h3 className="text-base font-semibold text-gray-900">Assign Category</h3>
              <button onClick={() => setEditingTxnId(null)} className="text-gray-400 hover:text-gray-600 cursor-pointer p-1 rounded-lg hover:bg-gray-100 transition-colors">
            ...
            <div className="space-y-4">
              <div>
                ...
                <StyledSelect
                  value={editParentId}
                  onChange={(v) => { setEditParentId(v); setEditSubId(''); }}
                >
